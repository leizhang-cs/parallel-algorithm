...
public:
    ...
    ivec2 number_pixels; // number of pixels: x and y direction, (width, height)
    ...
    vec3 World_Position(const ivec2& pixel_index, const ivec2& cell_index, const int n);
    ...
    vec2 Cell_Postion(const ivec2& index, const ivec2& cell_index, const int n) const
    {
        vec2 offset;
        offset[0] = (cell_index[0]+(double)rand()/RAND_MAX/2.0)/n;
        offset[1] = (cell_index[1]+(double)rand()/RAND_MAX/2.0)/n;
        return min+(vec2(index)+offset)*pixel_size;
    }

    Ray Set_Ray(const ivec2& pixel_index);
    void Set_Ray(const ivec2& pixel_index, std::vector<Ray>& ray, int samples);
